...
#setwd('/Users/gra38/Library/CloudStorage/Box-Box/Repositories/NYPollinatorData/folder-name-here')
...
build_form_ui <- function(id, fields, title = "Data Entry Form") {
  ns <- NS(id)
...
datasheet_fields <- list(
  ...
  list(id = "site", type = "text", label = "Site"),
  list(id = "species", type = "select", label = "Species",
       choices = c("Bee", "Fly", "Butterfly")),
  list(id = "count", type = "numeric", label = "Count")
)
...
# ---- APP ----
ui <- fluidPage(
  build_form_ui("form1", datasheet_fields, title = "Pollinator Datasheet")
)
